...
- "kimi 零 算一下" → 路由到 `kimi0`
- "kimi 二 做个任务" → 路由到 `kimi2`
...
bridge 会把这条纯图片消息暂存成当前聊天里的一个待分配图片批次。下一条
`@agent ...` 会认领这一批图片并附加到 prompt 里；bridge 不会再从全局
`incoming_media/` 猜“最近几张图”。

如果当前聊天里同时堆了多批未认领图片，bridge 会失败关闭并要求你重发，
不会把旧图错挂到新消息上。
